                ...
                <div class="col-12 text-center">
                    ...
                    <?php
                       ...
                       if (isset($res) && $res->num_rows > 0) {
                           ...
                           // Handle the case where no rows are returned, set a default value or display an error message
                           // For example:
                           $user_start_weight_date = ""; // Set a default value
                           echo "<h4>No data found for user's start weight date.</h4>"; // Display an error message
                       }
                    ?>
                </div>
            </div>
...
<div class="col-lg-4">
    <?php
        // Initialize $dailyCalneeded
        $dailyCalneeded = 0;

        // Check if the necessary variables are numeric
        if(is_numeric($newWeight) && is_numeric($user_height) && is_numeric($user_age)){
            // Calculate $dailyCalneeded based on gender
            if($user_gender == 0){
    ...
    <div class="card highlighter factCard
    <?php
        // Check if $dailyCalneeded is within a certain range and apply appropriate CSS class
        if($dailyCalneeded >= 1000){
            echo "alert-success";
        } else if($dailyCalneeded < 1000 && $dailyCalneeded >= 800){
            echo "alert-primary";
        } else {
            echo "alert-danger";
        }
    ?>">
        <div class="col-12">
            <b>
                ...
                ?>
            </b> <a href="#" data-toggle="tooltip" data-placement="right" title="With this amount of calories per day you'll achieve your weight goal on the chosen day">
                <i class="far fa-question-circle"></i>
            </a>
        </div>
        <div class="col-12 text-center">
            <h2>
                <?php
                    // Output the calculated value or "Missing entry" if it's not set
                    if (isset($dailyCalneeded)) {
                        echo round($dailyCalneeded);
                    } else {
                        echo "Missing entry";
                    }
                ?>
            ...
            <?php
                ...
                if (isset($dailyCalneeded) && is_numeric($dailyCalneeded)) {
                    echo "Your daily basal metabolism: " . $dailyCalneeded . " Calories";
                }
            ?>
        </div>
    </div>
        ...
        <div class="col-lg-4">
            <?php
                ...
                if(isset($dailyCalneeded) && is_numeric($dailyCalneeded)){
                    ...
                    $echo = $dailyCalneeded - $caloriesUsed;
